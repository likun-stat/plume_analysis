...
# Training wrapper
# ----------------------------
train_vae <- function(X, latent_dim = 50, hidden_dim = 100,
...
# ----------------------------
# Example usage (assuming you already built X)
# ----------------------------
# The following is great!
# model <- train_vae(
#   log(X),
#   latent_dim = 100, hidden_dim = 1000,
#   batch_size = 100, epochs = 20000,
...
for(beta in c(0.1, 0.5, 1)){
  if(beta==1) {name = ""} else if(beta==0.5) {name="50"} else{name="10"}
  for(ncomp in c(7,50,90)){
    model <- train_vae(
      log(X),
      latent_dim = ncomp, hidden_dim = 50,
    ...
    # Example:
    X_emu <- array(NA, dim=dim(X))
    for(ind in 1:ncol(X)){
      out <- emulate_time(model, log(X), t_idx = ind, n_draws = 10)
    ...
    save(fire_vanilla_VAE, file=paste0("./emulation_vanilla_VAE_", name, "_ncomp_",ncomp,".RData"))
  }
}


tmp_range <- range((X[,ind]))
xhat_det <- X_emu[,ind]
ggplot(stations) + geom_raster(aes(x=x, y=z, fill=exp(xhat_det))) +
