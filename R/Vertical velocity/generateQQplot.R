...
load("./CV_metrix_k_100fold1.RData")

dim(center_fire)

pdf(file = "./Figures/qqplot_fold1_K_50_add_data.pdf", width = 4.5, height = 4.8)
lims <- c(0,14.5)
extRemes::qqplot(center_fire[,1:10], fire_approx, regress = FALSE,  
                 ylim=lims, xlim=lims,
                 ylab=expression(paste('Emulated ', bolditalic(X)[1:10])), 
                 xlab=expression(paste('Observed vertical velocity', bolditalic(X)[1:10])), 
                 main = expression(italic(K)==50))
dev.off()


load("./emulation_fire_1.RData")
pdf(file = "./Figures/qqplot_fold1_K_100_add_data.pdf", width = 4.5, height = 4.8)
lims <- c(0,14.5)
extRemes::qqplot(center_fire[,21:30], fire_approx[,21:30], regress = FALSE,  
                 ylim=lims, xlim=lims,
                 ylab=expression(paste('Emulated ', bolditalic(X)[1:10])), 
                 xlab=expression(paste('Observed vertical velocity ', bolditalic(X)[1:10])), 
                 main = expression(italic(K)==100))
dev.off()


pdf(file = "./Figures/qqplot_fold1_K_500_add_data.pdf", width = 4.5, height = 4.8)
lims <- c(0,14.5)
extRemes::qqplot(center_fire[,1:10], fire_approx[,1:10], regress = FALSE,  
                 ylim=lims, xlim=lims,
                 ylab=expression(paste('Emulated ', bolditalic(X)[1:10])), 
                 xlab=expression(paste('Observed vertical velocity ', bolditalic(X)[1:10])), 
                 main = expression(italic(K)==500))
dev.off()
...
extRemes::qqplot(center_fire[,91:100], fire_approx, regress = FALSE,  
                 ...
                 xlab=expression(paste('Observed vertical velocity', bolditalic(X)[91:100])), 
