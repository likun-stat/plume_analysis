...
chi_plot <- function(X, stations, emulation, distance, tol=0.001,
                     ...
                     show.axis.y = TRUE, duplicated = TRUE, check_max_stab=FALSE){
  require(fields)
  require(ggplot2)
  d <- distance
  upper <- d+tol
  ...
  if(check_max_stab) {
    dat <- data.frame(x=u_vec,truth=(EmpIntv_sim[,3]-tail(EmpIntv_sim[,3],1))/(1-u_vec),
                      truth_upper=(1.05*EmpIntv_sim[,2]-tail(EmpIntv_sim[,3],1))/(1-u_vec),
                      truth_lower=(0.95*EmpIntv_sim[,1]-tail(EmpIntv_sim[,3],1))/(1-u_vec),
                      emu=(EmpIntv_emu[,3]-tail(EmpIntv_emu[,3],1))/(1-u_vec),
                      emu_upper=(1.1*EmpIntv_emu[,2]-tail(EmpIntv_emu[,3],1))/(1-u_vec),
                      emu_lower=(0.9*EmpIntv_emu[,1]-tail(EmpIntv_emu[,3],1))/(1-u_vec))
    cat(tail(EmpIntv_sim[,3],1), "\n")
    cat(dat$emu, "\n")
    plt <- ggplot(dat,aes(x=x,y=truth)) +
      geom_line(aes(color="Truth"),linewidth=0.7) +
      geom_line(aes(y=emu,color="Emulation"),linewidth=0.7) +
      scale_color_manual(values=c('red', 'black')) + 
      geom_ribbon(data=dat,aes(ymin=emu_lower,ymax=emu_upper),alpha=0.2,fill="red") +
      geom_ribbon(data=dat,aes(ymin=truth_lower,ymax=truth_upper),alpha=0.4,fill="black") +
      labs(colour="Type") +
      ylab((expression((chi[u]-chi)/(1-u)))) + xlab("Quantile") + 
      ggtitle(paste("Distance in (", lower, ", ", upper, ")", sep=''))+
      theme(plot.title = element_text(hjust = 0.5)) + 
      scale_x_continuous(expand = c(0, 0)) +
...
# Adaptive, data-dependent activation:
# - For elements x < thresh: linearly rescale to (0.5, out_max)
# - For elements x >= thresh: leave as-is
# * If no entries are < thresh, returns x unchanged.
custom_activation <- nn_module(
  "adaptive_threshold_rescale",
  initialize = function(thresh = 3, out_min = 0.5, out_max = 1, eps = 1e-8, learn_stats = FALSE) {
    self$thresh <- thresh
  ...
  forward = function(x) {
    mask <- x$lt(self$thresh)
    
    # If no entries satisfy the condition, return x unchanged
    if (!mask$any()$item()) {
      return(x)
    }
    ...
    # Rescale x_sub → [out_min, out_max]
    scale01 <- (x_sub - xmin) / (xmax - xmin + self$eps)
    y_sub <- self$out_min + (self$out_max - self$out_min) * scale01
