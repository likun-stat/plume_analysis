...
bottom_xs <- seq(2, 37, by=2*k)
bottom_ys <- seq(1, 600, by=2*k)

top_xs <- seq(2+k, 37, by=2*k)
...
U_PCA <- U_all[, (2*ncol(center_fire)+1):(3*ncol(center_fire))]
...
dat[dat>1] <- 1
dat[dat<0] <- 0
dat_ALL <- rbind(dat_ALL, cbind(dat, source="xVAE, K=100"))
...
dat[dat>1] <- 1
dat[dat<0] <- 0
dat_ALL <- rbind(dat_ALL, cbind(dat, source="POD, K=200"))
...
plt <- ggplot(dat_ALL,aes(x=x,y=truth, group=source, colour=source, fill=source)) +
  ...
  ylab(expression(chi[italic(d)](italic(p)))) + xlab("Quantile") + labs(fill="Vertical velocity", colour="Vertical velocity")+
  theme(plot.title = element_text(hjust = 0.5), axis.title.y = element_blank(), legend.title = element_text(face = "bold")) + 
  scale_x_continuous(expand = c(0, 0), limits=c(0.9,1.0001)) + ggtitle(paste0("d=",round(d,2))) +
  scale_y_continuous(expand = c(0, 0), limits = c(0,1)) + 
  force_panelsizes(rows = unit(3.05, "in"),
                   cols = unit(3.05, "in"))
plt
ggsave("./Figures/chi_d_8_add_data.png",  width = 5, height = 4)
...
top_xs <- seq(2+floor(k), 37, by=2*k)
if(length(top_xs)!=length(bottom_xs)){
  retain_xs <- min(length(top_xs), length(bottom_xs))
  bottom_xs <- bottom_xs[1:retain_xs]
  top_xs <- top_xs[1:retain_xs]
}
top_ys <- seq(1+floor(k), 600, by=2*k)
...
plt <- ggplot(dat_ALL,aes(x=x,y=truth, group=source, colour=source, fill=source)) +
  ...
  force_panelsizes(rows = unit(3.05, "in"),
                   cols = unit(3.05, "in"))
plt
ggsave("./Figures/chi_d_4_add_data.png",  width = 4, height = 4)
...
##-----------------------------------------------------------------------------
## ------------------ Didn't change for new application -----------------------
##-----------------------------------------------------------------------------
##-----------------------------------------------------------------------------
## ------------------ Didn't change for new application -----------------------
##-----------------------------------------------------------------------------
